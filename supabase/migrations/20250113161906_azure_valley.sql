...
DROP POLICY IF EXISTS "Users can view own settings" ON users_settings;
DROP POLICY IF EXISTS "Users can update own settings" ON users_settings;
DROP POLICY IF EXISTS "Users can insert own settings" ON users_settings;
...
CREATE POLICY "Users can insert own settings" ON users_settings
  FOR INSERT WITH CHECK (auth.uid() = user_id);
